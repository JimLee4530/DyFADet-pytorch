...
from .blocks_cloformer import SGPBlock_cloformer

@register_backbone("SGP_clo")
class SGP_cloBackbone(nn.Module):
    """
    ...
    def __init__(
            ...
            cloformer_kersizes=[5, 7],  # kernel sizes for cloformer
    ):
        super().__init__()
        assert len(arch) == 3
        ...
        for idx in range(arch[1]):
            self.stem.append(
                SGPBlock_cloformer(n_embd, cloformer_kersizes, 1, n_hidden=sgp_mlp_dim, k=k, init_conv_vars=init_conv_vars))

        # main branch using transformer with pooling
        self.branch = nn.ModuleList()
        for idx in range(arch[2]):
            self.branch.append(SGPBlock_cloformer(n_embd, cloformer_kersizes, self.scale_factor, path_pdrop=path_pdrop,
